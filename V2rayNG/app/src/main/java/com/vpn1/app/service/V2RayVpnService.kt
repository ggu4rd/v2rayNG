...
class V2RayVpnService : VpnService(), ServiceControl {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        Log.d("XrayDebug", "onStartCommand()")
    ...
    /** Sets up the VPN service. Prepares the VPN and configures it if preparation is successful. */
    private fun setup() {
        Log.d("XrayDebug", "setup() 0")

        val prepare = prepare(this)
        Log.d("XrayDebug", "setup() 1")

        if (prepare != null) {
            Log.d("XrayDebug", "setup() 2")

            return
        }

        if (setupVpnService() != true) {
            Log.d("XrayDebug", "setup() 3")
    ...
    private fun setupVpnService(): Boolean {
        Log.d("XrayDebug", "setupVpnService()")

        // If the old interface has exactly the same parameters, use it!
        // Configure a builder while parsing the parameters.
        val builder = Builder()
        // val enableLocalDns = defaultDPreference.getPrefBoolean(AppConfig.PREF_LOCAL_DNS_ENABLED,
        // false)
    ...
    /** Runs the tun2socks process. Starts the tun2socks process with the appropriate parameters. */
    private fun runTun2socks() {
        Log.d("XrayDebug", "runTun2socks()")

        val socksPort = SettingsManager.getSocksPort()
        val cmd =
                arrayListOf(
                        File(applicationContext.applicationInfo.nativeLibraryDir, TUN2SOCKS)
                                .absolutePath,
                        "--netif-ipaddr",
    ...
    /**
     * Sends the file descriptor to the tun2socks process. Attempts to send the file descriptor
    ...
    private fun stopV2Ray(isForced: Boolean = true) {
        ...
        //        val info = loadVpnNetworkInfo(configName, emptyInfo)!! + (lastNetworkInfo ?:
        // emptyInfo)
